class Plot {
    ...
    prepareSvg() {
        ...
        for(let i=labels.length-1, x=svgX+svgWidth-5-xAxisW, y=svgY+svgHeight-30/2; i >= 0; --i) {
            ...
            const labelText = this.createTextFromLabel(labels[i], x+25, y, '#1c3970');
        ...
        for(let i=0, m=min, x=svgX+svgWidth-5, y=svgY+svgHeight-30/2; i <= yAxisScale; ++i) {
            ...
            const labelText = this.createTextFromLabel(m.toString(), x-svgWidth+30/2, y-5, '#1c3970');
            this.svg.appendChild(labelText);
            m += res_bound;
        ...
        for(let i=labels.length-1, x=svgX+svgWidth-5-xAxisW, y=svgY+svgHeight-30/2; i >= 0; --i) {
            ...
            rect.setAttributeNS(null, 'fill', '#3db3e5');
